    ...
    <div className="w-[22rem] border-l border-border/50 hidden xl:flex xl:flex-col bg-background shadow-[-1px_0_10px_rgba(0,0,0,0.02)] z-10">
      <Tabs value={activeTab} onValueChange={handleTabChange} className="flex flex-col h-full">
        ...
        <TabsContent value="transcript" className="flex-1 flex flex-col overflow-hidden m-0">
          {/* Fullscreen Overlay Wrapper (Only active if isVideoPopupOpen) */}
          <div className={isVideoPopupOpen ? "fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md" : "flex flex-col flex-1 overflow-hidden"}>
            
            {/* Theater Container */}
            <div className={isVideoPopupOpen ? "w-[95vw] h-[92vh] max-w-[1600px] bg-background rounded-2xl overflow-hidden shadow-2xl border border-border flex flex-row relative" : "flex flex-col flex-1 overflow-hidden relative"}>
              
              {/* Left Side (Video) */}
              {isVideoFile && (
                <div className={isVideoPopupOpen ? "w-[65%] bg-black relative flex flex-col group border-r border-border" : "p-4 bg-muted/10 border-b border-border/40 shrink-0"}>
                  <div 
                    className={isVideoPopupOpen ? "w-full h-full relative flex items-center justify-center bg-black" : "relative overflow-hidden bg-black shadow-md rounded-2xl aspect-video ring-1 ring-border/50 group"}
                  ...
                  >
                    ...
                    {subtitlesEnabled && currentMiniSegment && (
                      <div className={`absolute left-0 right-0 flex justify-center px-8 pointer-events-none z-20 ${isVideoPopupOpen ? 'bottom-24' : 'bottom-12'}`}>
                        <div className={`backdrop-blur-sm text-white rounded-lg text-center drop-shadow-md ${isVideoPopupOpen ? 'bg-black/85 px-6 py-3 max-w-[85%] text-base shadow-xl' : 'bg-black/60 px-4 py-2 max-w-full text-sm font-medium'}`}>
                          {isVideoPopupOpen && (
                            <span className="text-primary font-semibold">{currentMiniSegment.speaker}: </span>
                          )}
                          <span className={isVideoPopupOpen ? "leading-relaxed" : ""}>{currentMiniSegment.text}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div 
                      className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent transition-opacity duration-300 ${!isVideoPopupOpen ? 'p-3 opacity-0 group-hover:opacity-100' : `px-6 py-5 ${controlsVisible || !isPlaying ? 'opacity-100' : 'opacity-0'}`}`}
                      onClick={(e) => e.stopPropagation()}
                    >
                      {/* Progress Bar */}
                      <div 
                        className={`w-full bg-white/20 rounded-full cursor-pointer group/bar transition-all ${isVideoPopupOpen ? 'mb-4 h-1.5 hover:h-2.5' : 'mb-2.5 h-1.5 hover:h-2'}`}
                        onClick={(e) => {
                      ...
                      >
                        ...
                        >
                          <div className={`absolute right-0 top-1/2 -translate-y-1/2 bg-white rounded-full opacity-0 group-hover/bar:opacity-100 shadow-md scale-0 group-hover/bar:scale-100 transition-all duration-200 ${isVideoPopupOpen ? 'w-4 h-4' : 'w-3.5 h-3.5'}`} />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-white">
                        <div className="flex items-center gap-1.5">
                          <button 
                            ...
                            className={`hover:bg-white/20 rounded-full transition-colors ${isVideoPopupOpen ? 'p-2.5' : 'p-1.5'}`}
                          >
                            {isPlaying ? (
                              <IconPlayerPause className={isVideoPopupOpen ? 'h-6 w-6' : 'h-4 w-4'} />
                            ) : (
                              <IconPlayerPlay className={isVideoPopupOpen ? 'h-6 w-6' : 'h-4 w-4'} />
                            )}
                          </button>
                          <button 
                            onClick={toggleMute}
                            className={`hover:bg-white/20 rounded-full transition-colors ${isVideoPopupOpen ? 'p-2.5 ml-1' : 'p-1.5'}`}
                          >
                            {isMuted ? (
                              <IconVolume3 className={isVideoPopupOpen ? 'h-5 w-5' : 'h-4 w-4'} />
                            ) : (
                              <IconVolume className={isVideoPopupOpen ? 'h-5 w-5' : 'h-4 w-4'} />
                            )}
                          </button>
                          <span className={`font-medium opacity-80 tabular-nums ${isVideoPopupOpen ? 'text-sm ml-3' : 'text-[10px] ml-1'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          <button 
                            ...
                            className={`hover:bg-white/20 rounded-full transition-colors relative ${isVideoPopupOpen ? 'p-2.5' : 'p-1.5'} ${subtitlesEnabled && isVideoPopupOpen ? 'bg-primary/30 text-primary' : subtitlesEnabled ? 'text-white' : 'text-white/50'}`}
                            title={subtitlesEnabled ? "Nonaktifkan Subtitle" : "Aktifkan Subtitle"}
                          >
                            <IconSubtitles className={isVideoPopupOpen ? 'h-6 w-6' : 'h-5 w-5'} />
                            {!subtitlesEnabled && (
                              <span className={`absolute w-0.5 bg-current rotate-45 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${isVideoPopupOpen ? 'h-7 bg-white' : 'h-5'}`} />
                            )}
                          </button>
                          <button 
                            onClick={() => setIsVideoPopupOpen(!isVideoPopupOpen)}
                            className={`hover:bg-white/20 rounded-full transition-colors text-white/80 hover:text-white ${isVideoPopupOpen ? 'p-2.5' : 'p-1.5'}`}
                            title={isVideoPopupOpen ? "Perkecil Video" : "Perbesar Video"}
                          >
                            {isVideoPopupOpen ? <IconMinimize className="h-6 w-6" /> : <IconMaximize className="h-5 w-5" />}
                          </button>
                        </div>
                      </div>
                    </div>
              ...
              <div className={isVideoPopupOpen ? "w-[35%] flex flex-col bg-background" : "flex flex-col flex-1 overflow-hidden"}>
                
                {/* Header specifically for fullscreen mode */}
                {isVideoPopupOpen && (
                  <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-muted/30 shrink-0">
                    ...
                    <button onClick={() => setIsVideoPopupOpen(false)} className="p-2 hover:bg-muted rounded-lg transition-colors bg-background border border-border shadow-sm">
                ...
                {!isVideoPopupOpen && (
                  <div className="px-4 py-4 space-y-3 bg-background relative z-10 shadow-[0_4px_10px_rgba(0,0,0,0.02)] shrink-0">
                ...
                <div ref={transcriptContainerRef} className={`flex-1 overflow-y-auto px-4 space-y-4 pb-24 ${isVideoPopupOpen ? 'pt-4 bg-background' : 'pt-4 bg-muted/10'}`}>
